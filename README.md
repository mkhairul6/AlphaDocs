...
Notifications are configured to be pushed to a user's mobile device a few minutes before the order reaches PREPARATION status. This can be managed using the TIME TO NOTIFY CUSTOMER BEFORE KITCHEN PREPARATION (MINUTES) field in the backend for editing store properties.
