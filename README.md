...
`http://<server>/ipg/mpgs/mobilePayment/<uniquePaymentId>?brand=<brandCode>&amount=<paymentAmount>`
...
* If the url contains `mobileProcessing` - payment is being processed by our sever waiting for a notification. Keep reloading the same URL (maybe once every 3-5 seconds) until the web view gets redirected to one containing the key word `mobileSuccess`. Allow for a timeout after 30 seconds.
